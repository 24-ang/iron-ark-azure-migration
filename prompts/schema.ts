...
# 【数据结构定义】Wasteland Stalker SaveData (V4.0 Native)
...
- \`gameState.当前日期\`: String ("2477-01-01")
- \`gameState.当前地点\`: String (中文地名，如 "钢铁方舟核心区")
- \`gameState.当前区域\`: Number (Zone ID)
- \`gameState.天气\`: String ("晴朗", "辐射风暴", "酸雨" 等)
...
- \`gameState.角色.种族\`: String ("人类", "新人类", "机械体", "变异人")
- \`gameState.角色.所属势力\`: String
- \`gameState.角色.等级\`: Number (Level)
- \`gameState.角色.外貌\`: String
- \`gameState.角色.背景\`: String
- \`gameState.角色.性别\`: String ("男性" / "女性" / "无性")
- \`gameState.角色.生理类型\`: String ("Organic" / "Construct" / "Hybrid")
...
- \`gameState.角色.精神力\`: Number (当前 MP/Energy)
- \`gameState.角色.最大精神力\`: Number
- \`gameState.角色.体力\`: Number (当前 Stamina)
- \`gameState.角色.最大体力\`: Number
- \`gameState.角色.辐射值\`: Number (当前 Rads)
- \`gameState.角色.最大辐射值\`: Number
...
**改造能力值 (Augment Stats)**
*范围 0-999+*
- \`gameState.角色.能力值.力量\` (STR) - 液压/肌肉输出
- \`gameState.角色.能力值.耐久\` (END) - 装甲/骨骼强度
- \`gameState.角色.能力值.灵巧\` (DEX) - 火控/精细操作
- \`gameState.角色.能力值.敏捷\` (AGI) - 引擎/反应速度
- \`gameState.角色.能力值.能源\` (NRG) - 核心功率/电池

**资源与成长**
- \`gameState.角色.经验值\`: Number (Combat Data)
- \`gameState.角色.配给券\`: Number (Credits/Money)
- \`gameState.角色.黑匣子数据\`: Number (Blackbox Data - 原伟业)
- \`gameState.角色.升级所需数据\`: Number
- \`gameState.角色.疲劳度\`: Number (0-100)
- \`gameState.角色.公会评级\`: String ("I" 到 "S")

**技能与程序**
- \`gameState.角色.程序槽位.上限\`: Number
- \`gameState.角色.程序槽位.已使用\`: Number
- \`gameState.角色.程序槽位.扩展来源\`: String[]
- \`gameState.角色.发展能力\`: Array<{ "名称", "等级", "类型", "描述", "效果", "解锁条件", "备注" }>
- \`gameState.角色.技能\`: Array<{ "id", "名称", "类别", "描述", "效果", "触发", "持续", "冷却", "消耗", "范围", "命中", "适用", "等级", "关联能力", "限制", "标签", "稀有"(Bool), "备注" }>
- \`gameState.角色.战术程序\`: Array<{ "id", "名称", "启动指令", "类别", "属性", "描述", "效果", "范围", "射程", "冷却", "消耗", "运行条件", "标签", "稀有"(Bool), "备注" }>
- \`gameState.角色.状态\`: Array<{ "名称", "类型": "Buff" | "DeBuff", "效果", "结束时间" }>
- \`gameState.角色.异常\`: Array<{ "名称", "类型": "Buff" | "DeBuff", "效果", "结束时间" }>
...
- \`描述\`: String
- \`数量\`: Number
- \`类型\`: String ("consumable" | "weapon" | "armor" | "material" | "key_item" | "loot" | "chip")
- \`获取途径\`: String ("wasteland" | "public")
- \`品质\`: String ("Scrap" | "Common" | "Rare" | "Epic" | "Legendary")
- \`标签\`: String[] | String
- \`来源\`: String
- \`制造商\`: String
- \`材质\`: String
...
- \`gameState.世界.公会声望\`: Number
- \`gameState.世界.头条新闻\`: String[]
- \`gameState.世界.街头传闻\`: Array<{ "主题": String, "传播度": Number }>
- \`gameState.世界.废土董事会\`: { "下次会议开启时间": String, "会议主题": String, "讨论内容": Array<{ "角色": String, "对话": String }>, "最终结果": String }
- \`gameState.世界.NPC后台跟踪\`: Array<{ "NPC": String, "当前行动": String, "位置?": String, "进度?": String, "预计完成?": String }>
- \`gameState.世界.势力格局\`: { "S级": String[], "A级": String[], "B级至I级": String[], "备注": String }
- \`gameState.世界.区域战争\`: { "状态": String, "参战势力": String[], "形式": String, "争夺目标": String, "开始时间": String, "结束时间": String, "结果": String, "备注": String }
...
- \`gameState.地图.wastelandZones\`: Array<{ "zoneStart", "zoneEnd", "name", "description", "dangerLevel", "landmarks" }>
...
- \`gameState.剧情.路线\`: { "类型": String, "偏移度": Number, "分支说明?": String }
...
## 13. 势力信息 (gameState.势力)
- \`gameState.势力.名称\`: String
- \`gameState.势力.等级\`: String
- \`gameState.势力.首领\`: String
- \`gameState.势力.资金\`: Number
- \`gameState.势力.设施状态\`: Object
- \`gameState.势力.仓库\`: Array<InventoryItem>
</数据结构定义>`;
