...
import { WastelandStats, IdentityRoute, FirmwareVersion } from './character';
...
export interface Confidant {
  ...
  性别?: string;
  机体型号?: string; // Was 种族
  身份路线?: IdentityRoute | string; // Was 身份
  运行时间?: number; // Was 年龄
  所属势力: string; // Was 眷族: Replace with 'Faction' or 'Shelter' concept
  ...
  机体版本?: FirmwareVersion | string; // Was 等级
  已知能力?: string; // knownAbilities

  // 战斗数值 (Vitals)
  生存数值?: {
      当前生命: number; 最大生命: number;
      当前负载?: number; 最大负载?: number; // Was 精神
      当前体力: number; 最大体力: number;
  };

  // 基础能力 (Stats)
  属性?: Partial<WastelandStats>; // Was 能力值
