...
import { GameState, AppSettings, LogEntry, InventoryItem, TavernCommand, ActionOption, PhoneMessage, PhoneThread, PhonePendingMessage, Confidant, MemorySystem, MemoryEntry, SaveSlot, Task, ContextModuleConfig, PhonePost, PhoneAIResponse, WorldState } from '../types';
import { createNewGameState } from '../utils/dataMapper';
import { generateGameMasterResponse, generatePhoneResponse, generateWorldInfoResponse, DEFAULT_PROMPT_MODULES, DEFAULT_MEMORY_CONFIG, dispatchAIRequest, generateMemorySummary, extractThinkingBlocks, parseAIResponseText, mergeThinkingSegments, resolveServiceConfig } from '../utils/ai';
...
const DEFAULT_CONTEXT_MODULES: ContextModuleConfig[] = [
    ...
    { id: 'm_faction', type: 'FACTION_CONTEXT', name: '势力信息', enabled: true, order: 5, params: {} },
...
// NEW: Migration for Wasteland Transition
const migrateToWasteland = (state: GameState): GameState => {
    if (!state) return state;
    let nextState = { ...state };
    let worldChanged = false;

    // 1. Migrate World State
    const world = nextState.世界 || {} as any;
    const newWorld: any = { ...world };

    if (world.眷族声望 !== undefined && world.公会声望 === undefined) {
        newWorld.公会声望 = world.眷族声望;
        delete newWorld.眷族声望;
        worldChanged = true;
    }
    if (world.诸神神会 && !world.废土董事会) {
        newWorld.废土董事会 = {
            下次会议开启时间: world.诸神神会.下次神会开启时间 || "未知",
            会议主题: world.诸神神会.神会主题 || "待定",
            讨论内容: world.诸神神会.讨论内容 || [],
            最终结果: world.诸神神会.最终结果 || "待议"
        };
        delete newWorld.诸神神会;
        worldChanged = true;
    }
    if (world.派阀格局 && !world.势力格局) {
        newWorld.势力格局 = world.派阀格局;
        delete newWorld.派阀格局;
        worldChanged = true;
    }
    if (world.战争游戏 && !world.区域战争) {
        newWorld.区域战争 = {
            状态: world.战争游戏.状态 || "未开始",
            参战势力: world.战争游戏.参战眷族 || [],
            形式: world.战争游戏.形式 || "待定",
            争夺目标: world.战争游戏.赌注 || "未公开",
            开始时间: world.战争游戏.举办时间,
            结束时间: world.战争游戏.结束时间,
            结果: world.战争游戏.结果,
            备注: world.战争游戏.备注
        };
        delete newWorld.战争游戏;
        worldChanged = true;
    }

    if (worldChanged) {
        nextState.世界 = newWorld as WorldState;
    }

    // 2. Migrate Map (If legacy Orario map detected or missing Wasteland zones)
    // We check if 'wastelandZones' is missing or if 'districts' exists
    const map = nextState.地图 as any;
    if (map && (!map.wastelandZones || map.districts)) {
        // Regenerate Map to Wasteland
        // We need to import generateWastelandMap but it's not imported here. 
        // We can't easily import it inside function. 
        // Alternative: We will handle this in the component or just patch missing array to avoid crash
        if (!map.wastelandZones) {
             const patchMap = { ...map, wastelandZones: [] }; // Prevents crash
             if (map.districts) {
                 // Convert districts to zones strictly for UI safety
                 patchMap.wastelandZones = map.districts.map((d: any) => ({
                     zoneStart: d.startFloor || 1,
                     zoneEnd: d.endFloor || 1,
                     name: d.name,
             ...
             nextState.地图 = patchMap;
        }
    }

    // 3. Migrate Faction (Familia -> Faction)
    if (nextState.眷族 && !nextState.势力) {
         // FactionState is alias of FamiliaState, so just move it
         nextState.势力 = nextState.眷族 as any;
         // nextState.眷族 = null; // Keep for legacy if needed, or null it? 
         // Type definition says FactionState replaced FamiliaState, but we have fields in GameState
    }
    // Ensure '势力' is initialized if missing
    if (!nextState.势力) {
        nextState.势力 = { 名称: "无", 等级: "I", 领袖: "", 资金: 0, 设施状态: {}, 仓库: [] };
    }
...
export const useGameLogic = (initialState?: GameState, onExitCb?: () => void) => {
    const [gameState, setGameState] = useState<GameState>(() => {
        if (initialState) {
            ...
            return migrateToWasteland(migrateNpcActionsToTracking(initialState));
        }
        return migrateToWasteland(migrateNpcActionsToTracking(createNewGameState("Adventurer", "Male", "Human")));
    });
        ...
        if (savedSettings) {
            try {
                ...
                const mergedDefaults = DEFAULT_PROMPT_MODULES.map(def => {
                    ...
                    const renamed = renameMap[savedAny.name] ? def.name : savedAny.name;
                    // CRITICAL FIX: Always use the content from code (def.content), never from storage.
                    // Storage should only save on/off state (isActive) and maybe order.
                    return { ...def, ...savedAny, content: def.content, name: renamed };
                });
    ...
    useEffect(() => {
        if (gameState.回合数 > 1) {
            ...
            const saveData: SaveSlot = {
                ...
                type: 'AUTO',
                timestamp: Date.now(),
                summary: `AUTO: Lv.${optimizedState.角色.机体版本} ${optimizedState.当前地点}`,
                data: optimizedState,
                version: '3.0'
            };
            try {
    ...
    const manualSave = (slotId: number | string) => {
        ...
        const saveData: SaveSlot = {
            ...
            type: 'MANUAL',
            timestamp: Date.now(),
            summary: `MANUAL: Lv.${optimizedState.角色.机体版本} ${optimizedState.当前地点} ${optimizedState.游戏时间}`,
            data: optimizedState,
            version: '3.0'
        };
        try {
    ...
    const loadGame = (slotId: number | string) => {
        ...
        if (raw) {
            try {
                ...
                setGameState(migrateToWasteland(migrateNpcActionsToTracking(state)));
    ...
    const handleAIInteraction = async (
        ...
        try {
            ...
            const aiResponse = await generateGameMasterResponse(
                input, 
                stateWithUserLog, 
                settings, 
                "", 
    ...
    const confirmMemorySummary = async () => {
        ...
        try {
            ...
            const summaryResult = await generateMemorySummary(fakeLogs, memorySummaryState.type, settings);
            const normalizedSummary = normalizeSummaryText(summaryResult.summary);
