            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 pb-32 bg-[url('https://www.transparenttextures.com/patterns/diagmonds-light.png')] bg-zinc-950">
                {filteredItems.length > 0 ? filteredItems.map(item => {
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center justify-between mt-1">
                                    ...
                                    {item.已装备 && <span className="text-[9px] text-black font-bold bg-cyan-500 px-1 uppercase skew-x-[-10deg]">EQUIPPED</span>}
                                </div>
                            </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-blue-900">
                        ...
                        <span className="uppercase font-display tracking-widest">NO ITEMS</span>
                    </div>
                )}
            </div>
            ...
            {selectedItem && (
                <div className="absolute bottom-0 left-0 w-full bg-zinc-950 border-t-4 border-blue-600 p-5 shadow-[0_-10px_50px_rgba(0,0,0,0.9)] animate-in slide-in-from-bottom-full z-20 max-h-[70vh] flex flex-col">
                    ...
                    <div className="flex justify-between items-start mb-4 border-b border-blue-900 pb-3">
                        <div>
                            <div className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mb-1">SELECTED ITEM</div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-1">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs font-mono bg-black p-3 border border-blue-900/50 relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-1 bg-blue-900/30 border-l border-b border-blue-800 text-[9px] text-blue-300 uppercase">STATS</div>
                            
                            {selectedItem.攻击力 !== undefined && <span className="text-red-400 font-bold text-base">ATK {selectedItem.攻击力}</span>}
                            {selectedItem.防御力 !== undefined && <span className="text-blue-400 font-bold text-base">DEF {selectedItem.防御力}</span>}
                            {selectedItem.恢复量 !== undefined && <span className="text-green-400 font-bold text-base">REC {selectedItem.恢复量}</span>}
                            {selectedItem.价值 !== undefined && <span className="text-yellow-500 font-bold text-base">VAL {selectedItem.价值}</span>}
                            ...
                            <div className="space-y-2 bg-blue-900/10 p-3 border-l-2 border-purple-500">
                                {selectedItem.攻击特效 && selectedItem.攻击特效 !== "无" && (
                                    <div className="text-[10px] text-red-400 flex items-start gap-2">
                                        <span className="shrink-0 font-bold bg-red-900/30 px-1">EFFECT</span> 
                                        {selectedItem.攻击特效}
                                    </div>
                                )}
                                {selectedItem.效果 && (
                                    <div className="text-[10px] text-cyan-400 flex items-start gap-2">
                                        <span className="shrink-0 font-bold bg-cyan-900/30 px-1">PASSIVE</span> 
                                        {selectedItem.效果}
                                    </div>
                                )}
                            </div>
