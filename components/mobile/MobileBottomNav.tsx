...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ onTabSelect, activeTab, isHellMode }) => {
    ...
    return (
        <div className="h-16 bg-black border-t border-zinc-800 flex items-stretch shrink-0 z-50 pb-safe">
            <NavItem id="CHAT" icon={<MessageSquare size={20} />} label="CHAT" />
            <NavItem id="CHAR" icon={<User size={20} />} label="STATUS" />
            <NavItem id="INV" icon={<Briefcase size={20} />} label="GEAR" />
            <NavItem id="MAP" icon={<Map size={20} />} label="MAP" />
            <NavItem id="MENU" icon={<Menu size={20} />} label="MENU" />
        </div>
    );
};
