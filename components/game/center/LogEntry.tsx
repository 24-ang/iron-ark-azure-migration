...
export const LogEntryItem: React.FC<LogEntryProps> = ({ 
                ...
                {lines.map((line, idx) => {
                    ...
                    if (isJudge) {
                        return (
                            <div
                                key={idx}
                                className="px-2 py-1 border border-cyan-600/50 bg-cyan-950/40 text-cyan-200 font-mono tracking-widest text-[11px] uppercase"
                            >
                                {line}
                            </div>
                        );
        ...
        return (
            <div className={`mt-2 flex ${alignClass}`}>
                <div className="max-w-[90%] px-2 py-1 text-[10px] text-amber-200 border border-amber-700/60 bg-amber-900/30 uppercase tracking-wider">
                    Message Auto-Repaired: {log.repairNote}
                </div>
            </div>
        );
    };
    ...
    const MobileActions = ({ align }: { align: 'left' | 'right' | 'center' }) => {
        ...
        const alignClass = align === 'right' ? 'justify-end' : align === 'center' ? 'justify-center' : 'justify-start';
        return (
            <div className={`md:hidden mt-1 flex gap-2 ${alignClass} opacity-80 hover:opacity-100 transition-opacity`}>
                {canEditUser && (
                    <button
                        type="button"
                        onClick={(e) => {
                        ...
                        className="w-6 h-6 flex items-center justify-center rounded-sm border border-zinc-800 bg-black/40 text-zinc-500 hover:text-green-400 hover:border-green-500/50 hover:bg-green-900/20 transition-all"
                        title="Edit"
                    >
                        <Edit2 size={12} />
                    </button>
                ...
                {canDeleteUser && (
                    <button
                        ...
                        className="w-6 h-6 flex items-center justify-center rounded-sm border border-zinc-800 bg-black/40 text-zinc-500 hover:text-red-400 hover:border-red-500/50 hover:bg-red-900/20 transition-all"
                        title="Delete"
                    >
                        <Trash2 size={12} />
                    </button>
                    ...
                    `}>
                        ...
                        {canEditUser && (
                            <button 
                                ...
                                className="flex items-center gap-2 px-2 py-1.5 text-xs text-zinc-300 hover:text-white hover:bg-emerald-600 rounded transition-colors whitespace-nowrap"
                                title="Edit Content"
                            >
                                <Edit2 size={12} />
                            </button>
    ...
    // --- 2. NARRATOR (旁白/环境描写) ---
    if (isNarrator) {
        const bgGlow = isHellMode ? 'via-red-950/10' : 'via-emerald-950/10';
            ...
            <div className="group relative w-full my-1 animate-in fade-in duration-1000">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <div className="relative w-full md:w-[96%] px-4 md:px-4 py-3">
                        ...
                        <div className={`relative border ${isHellMode ? 'border-red-900/60' : 'border-green-900/60'} bg-black/40 px-4 md:px-6 py-4 shadow-sm`}>
        ...
        return (
            <div className="group relative flex w-full justify-end my-4 pl-10 animate-in slide-in-from-right-4 fade-in duration-300">
                ...
                <div className="flex items-end gap-3 max-w-full">
                    ...
                    <div className="bg-black border border-zinc-700 text-white px-4 py-3 rounded-2xl rounded-tr-none shadow-[0_4px_10px_rgba(0,0,0,0.5)] relative min-w-[60px] group-hover:border-emerald-500 transition-colors">
