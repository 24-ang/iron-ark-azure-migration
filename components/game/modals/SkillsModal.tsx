...
import { Skill, TechArt } from '../../../types';
...
const formatCost = (cost: any) => {
  ...
  if (typeof cost === 'object') {
    const parts: string[] = [];
    if (cost.Load !== undefined) parts.push(`LOAD ${cost.Load}`);
    if (cost.Stamina !== undefined) parts.push(`STM ${cost.Stamina}`);
    if (cost.Credits !== undefined) parts.push(`CREDITS ${cost.Credits}`);
    if (cost.精神 !== undefined && cost.精神 !== null && cost.精神 !== '') parts.push(`MP ${cost.精神}`);
    if (cost.体力 !== undefined && cost.体力 !== null && cost.体力 !== '' && cost.Stamina === undefined) parts.push(`STM ${cost.体力}`);
    if (cost.代价) parts.push(`COST ${cost.代价}`);
    return parts.join(' / ');
  }
  return String(cost);
};
...
export const SkillsModal: React.FC<SkillsModalProps> = ({ isOpen, onClose, skills, magic }) => {
  ...
  const renderSkillCard = (skill: Skill, index: number) => {
    ...
    const tags = [...normalizeTags(skill.标签), skill.稀有 ? 'RARE' : null].filter(Boolean) as string[];
    const metaItems = [
      { label: 'TYPE', value: skill.类别 },
      { label: 'RANK', value: skill.等级 },
      { label: 'TRIGGER', value: skill.触发 },
      { label: 'DURATION', value: skill.持续 },
      { label: 'CD', value: skill.冷却 },
      { label: 'RANGE', value: skill.范围 },
      { label: 'ACC', value: skill.命中 },
      { label: 'TARGET', value: skill.适用 },
      { label: 'LINK', value: skill.关联模块 }
        ...
        <div className="relative z-10 flex flex-col gap-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-xl text-white font-bold group-hover:text-blue-400 transition-colors">{skill.名称 || 'Unnamed Skill'}</h3>
            ...
            <div className="text-zinc-300 text-sm leading-relaxed">
              ...
              {skill.效果 && (
                <div className="mt-3 border border-blue-900/60 bg-blue-950/30 p-2 text-xs text-blue-100">
                  <div className="text-[10px] uppercase tracking-[0.3em] text-blue-300 mb-1">EFFECT</div>
          ...
          {(skill.限制 || skill.备注) && (
            <div className="text-[11px] text-zinc-500 border-t border-zinc-800 pt-3">
              {skill.限制 && <div>LIMIT: {formatValue(skill.限制)}</div>}
              {skill.备注 && <div>NOTE: {formatValue(skill.备注)}</div>}
            </div>
          )}
        </div>
      </div>
  ...
  const renderMagicCard = (spell: TechArt, index: number) => {
    const cost = formatCost(spell.消耗);
    const tags = [...normalizeTags(spell.标签), spell.稀有 ? 'RARE' : null].filter(Boolean) as string[];
    const metaItems = [
      { label: 'TYPE', value: spell.类别 },
      { label: 'ELEMENT', value: spell.属性 },
      { label: 'AREA', value: spell.范围 },
      { label: 'RANGE', value: spell.射程 },
      { label: 'CD', value: spell.冷却 },
      { label: 'COND', value: spell.施放条件 }
    ].filter(item => item.value !== undefined && item.value !== null && item.value !== '');
      ...
      <div key={spell.id || `${spell.名称}_${index}`} className="group relative bg-zinc-950 p-6 border border-zinc-800 hover:border-cyan-500 transition-all overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col gap-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-xl text-white font-bold group-hover:text-cyan-300 transition-colors">{spell.名称 || 'Unnamed Magic'}</h3>
          ...
          {spell.指令代码 && (
            <div className="border border-cyan-900/60 bg-cyan-950/30 p-2 text-[11px] text-cyan-100 leading-relaxed">
              <div className="text-[10px] uppercase tracking-[0.3em] text-cyan-300 mb-1">COMMAND SEQUENCE</div>
              <div className="whitespace-pre-wrap font-mono">{spell.指令代码}</div>
            </div>
          )}
            ...
            <div className="text-zinc-300 text-sm leading-relaxed">
              ...
              {spell.效果 && (
                <div className="mt-3 border border-cyan-900/60 bg-cyan-950/30 p-2 text-xs text-cyan-100">
                  <div className="text-[10px] uppercase tracking-[0.3em] text-cyan-300 mb-1">EFFECT</div>
  ...
  const emptyText = activeTab === 'SKILL' ? 'NO SKILLS LEARNED' : 'NO TACTICS RECORDED';
      ...
      <div className="w-full max-w-5xl bg-zinc-900 border-t-8 border-blue-600 relative shadow-[0_20px_50px_rgba(37,99,235,0.3)] max-h-[80vh] flex flex-col">
        <div className="p-6 flex flex-wrap items-end justify-between gap-4 border-b border-zinc-800 bg-gradient-to-r from-zinc-900 to-black">
          <div className="flex flex-col">
            <span className="text-blue-600 font-display text-lg uppercase tracking-widest">Skill & Tech</span>
            <h2 className="text-4xl md:text-5xl font-display uppercase tracking-wider text-white italic">SKILLS / TACTICS</h2>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {list.length > 0 ? (
              activeTab === 'SKILL'
                ...
                : list.map((spell, index) => renderMagicCard(spell as TechArt, index))
            ) : (
