      ...
      <div className="w-full max-w-6xl bg-[#0f0f12] border-4 border-amber-700 relative flex flex-col shadow-[0_0_50px_rgba(245,158,11,0.15)] max-h-[85vh]">
        <div className="bg-[#1b1b21] p-6 flex justify-between items-center border-b border-amber-900/60">
          <div className="flex items-center gap-4 text-amber-300">
            <Archive size={32} />
            <div>
              <h2 className="text-3xl font-display uppercase tracking-widest text-shadow-gold">LOOT VAULT</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {items.length > 0 ? items.map((item) => {
                ...
                <div key={item.id} className={`relative bg-black/70 border-2 ${style.border} p-4 flex flex-col gap-3 ${style.glow}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.价值 !== undefined && (
                        <div className="text-[10px] text-amber-300 font-mono mt-1">VAL: {item.价值}</div>
                      )}
                      {item.重量 !== undefined && (
                        <div className="text-[10px] text-zinc-500 font-mono">WT: {item.重量}</div>
                      )}
                    </div>
                  </div>
                  ...
                  {(item.效果 || item.攻击特效 || item.防御特效) && (
                    <div className="text-[10px] text-zinc-300 space-y-1">
                      {item.效果 && <div><span className="text-zinc-500">PASSIVE:</span> {item.效果}</div>}
                      {item.攻击特效 && item.攻击特效 !== '无' && <div><span className="text-zinc-500">EFFECT:</span> {item.攻击特效}</div>}
                      {item.防御特效 && item.防御特效 !== '无' && <div><span className="text-zinc-500">EFFECT:</span> {item.防御特效}</div>}
                    </div>
                  )}
              ...
              <div className="col-span-full flex flex-col items-center justify-center py-20 text-zinc-500">
                ...
                <span className="font-display text-2xl uppercase tracking-widest">VAULT EMPTY</span>
              </div>
            )}
          </div>
        </div>
