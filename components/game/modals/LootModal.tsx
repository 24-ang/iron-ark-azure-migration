      ...
      <div className="w-full max-w-5xl bg-[#1c1917] border-4 border-[#d4af37] relative flex flex-col shadow-[0_0_50px_rgba(212,175,55,0.2)] max-h-[85vh]">
        ...
        <div className="bg-[#292524] p-6 flex justify-between items-center border-b border-[#78350f]">
             <div className="flex items-center gap-4 text-[#d4af37]">
                <Archive size={32} />
                <div>
                    <h2 className="text-3xl font-display uppercase tracking-widest text-shadow-gold">SECTOR CACHE</h2>
                    <div className="text-xs font-mono text-[#a8a29e]">SECTOR CACHE</div>
                    {carrier && (
                        <div className="text-[10px] font-mono text-[#d4af37] mt-1">Carried by: {carrier}</div>
                    )}
                </div>
             </div>
                ...
                {items.length > 0 ? items.map((item) => {
                    ...
                    <div key={item.id} className={`relative bg-[#0c0a09] border-2 ${style.border} p-4 flex flex-col gap-3 hover:bg-[#1c1917] transition-all group ${style.glow}`}>
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="mt-2 flex flex-wrap items-center gap-2">
                                    <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 border ${item.获取途径 === 'wasteland' ? 'border-red-900 text-red-500' : 'border-blue-900 text-blue-500'}`}>
                                        {item.获取途径 === 'wasteland' ? 'WASTELAND' : 'OTHER'}
                                    </span>
                                    {item.价值 !== undefined && (
                                        <span className="text-[10px] text-[#d4af37] font-mono">VAL {item.价值}</span>
                                    )}
                                    {item.重量 !== undefined && (
                                        <span className="text-[10px] text-[#a8a29e] font-mono">WT {item.重量}</span>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-1 text-[10px] font-mono bg-black/40 p-2 border border-[#78350f]">
                            {item.攻击力 !== undefined && <span className="text-red-400">ATK {item.攻击力}</span>}
                            {item.防御力 !== undefined && <span className="text-blue-400">DEF {item.防御力}</span>}
                            {item.恢复量 !== undefined && <span className="text-green-400">REC {item.恢复量}</span>}
                            {item.等级需求 !== undefined && <span className="text-purple-300">REQ Lv.{item.等级需求}</span>}
                        </div>
                        ...
                        {(item.效果 || item.攻击特效 || item.防御特效) && (
                            <div className="text-[10px] text-[#d6d3d1] space-y-1">
                                {item.效果 && <div><span className="text-[#a8a29e]">PASSIVE:</span> {item.效果}</div>}
                                {item.攻击特效 && item.攻击特效 !== '无' && <div><span className="text-[#a8a29e]">EFFECT:</span> {item.攻击特效}</div>}
                                {item.防御特效 && item.防御特效 !== '无' && <div><span className="text-[#a8a29e]">EFFECT:</span> {item.防御特效}</div>}
                            </div>
                        )}

                        {item.魔剑 && (
                            <div className="text-[10px] text-purple-200 border border-purple-900/60 bg-purple-950/30 p-2 space-y-1">
                                <div className="uppercase text-purple-300 text-[9px]">MAGIC SWORD</div>
                                <div>Name: {item.魔剑.魔法名称 || item.名称}</div>
                                <div>Element: {item.魔剑.属性 || "??"} · PWR: {item.魔剑.威力 || "??"}</div>
                                {(item.魔剑.剩余次数 !== undefined || item.魔剑.最大次数 !== undefined) && (
                                    <div>Charges: {item.魔剑.剩余次数 ?? "?"}/{item.魔剑.最大次数 ?? "?"}</div>
                                )}
                            </div>
                        )}
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center py-20 text-[#57534e]">
                        ...
                        <span className="font-display text-2xl uppercase tracking-widest">CACHE EMPTY</span>
                    </div>
                )}
            </div>
        </div>
